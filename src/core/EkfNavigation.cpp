...
void EkfNavigation::initialize(const NavParamsBase& base_params, 
    ...
    ekf_.disA = MatrixXd::Identity(15, 15);
    ekf_.disQ = MatrixXd::Zero(15, 15);
}
...
void EkfNavigation::updateStrapdown(const IMUData& imu, int i) {
    ...
    // Store parameters for EKF prediction
    if (i == 0) {
...
void EkfNavigation::discretizeSystem(double dt) {
    ...
    G.block<3,3>(0,0) = state_.CtbM;  // Gyro bias to attitude error
...
void EkfNavigation::runEkfUpdate(int i, const Eigen::VectorXd& Z, const Eigen::MatrixXd& H) {
    ...
    // Reset error states
    ekf_.X.block(0,k,9,1).setZero();  // Reset first 9 states
...
// Strapdown velocity and position update
